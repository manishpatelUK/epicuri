...
;(function ( $, window, document, undefined ) {
	...
	"use strict";
		...
		var pluginName = "epicuribooking",
			defaults = {
				startStep: 1,
				steps: [
					{
						headerTitle: "Choose your party size and service",
						setupFunction: function(stepContent, plugin) {
							var reservationInfo = $("<div class='reservation-info'></div>");
							
							// number of guests select
							var numGuests = $("<select class='num-guests'></select>");
							plugin.settings.numGuestChoices.forEach(function (level) {
								numGuests.append($("<option value='" + level +"'>" + level + "</option>"));
							});
							plugin.reservation.numGuests = numGuests.val();
							numGuests.change(function() {
								plugin.reservation.numGuests = numGuests.val();
							});
							reservationInfo.append($("<label>" + plugin.settings.numberGuestsLabel + "</label>"));
							reservationInfo.append(numGuests);
							if (plugin.settings.biggerPartiesLabel != null && plugin.settings.biggerPartiesLabel != "")
							{
                                if(plugin.settings.biggerPartiesLabel.length < 40) {
                                    reservationInfo.append($("<br /><em class='hoverLabel'>" + plugin.settings.biggerPartiesLabel + "</em>"));
                                }
							}

							stepContent.append(reservationInfo);
							
							plugin.calendar = $("<div class='calendar'></div>");
							plugin.calendar.pickmeup({
								flat: true,
								format: 'Y-m-d',
								min: new Date(),
								change: function(formatted) {
									plugin.reservation.date = new Date(formatted);
								}
							});

							plugin.reservation.date = plugin.calendar.pickmeup("get_date");
							stepContent.append(plugin.calendar);
							
							var bookHereButton = $("<button class='right-bottom'>" + plugin.settings.dateStepButtonLabel + "</button>");
							bookHereButton.click(function() {plugin.displayStep(2)});
							stepContent.append(bookHereButton);
						}
					},
					{
						headerTitle: "What time would you like to book?",
						setupFunction: function(stepContent, plugin) {
							stepContent.append("<span>Content 2</span>");
							
							// Get the available time slots for the current time
							if (plugin.settings.stepsClickable) {
								var times = plugin.getReservationTimes();
								plugin.setReservationTime(times[0]);
							}
						},
						onLoad : function(contentDiv, plugin) {
							// Clear the div
							contentDiv.html("");
							...
							if (times.length == 0)
							{
								contentDiv.append("<h3>" + plugin.settings.noTimeSlotsMessage + "</h3>");
							}
							else
							{
								// Populate the div with those times
								times.forEach(function(time) {
									var timeBtn = $("<span class='time-slot'>" + time + "</span>");
									timeBtn.click(function() {
										plugin.setReservationTime(time);
									});
									
									contentDiv.append(timeBtn);
								});
								plugin.setReservationTime(times[0]);
							...
							var backButton = $("<button class='left-bottom'>" + plugin.settings.backLabel + "</button>");
							backButton.click(function() {plugin.displayStep(1)});
							contentDiv.append(backButton);

							if (times.length > 0) {
								var forwardButton = $("<button class='right-bottom'>" + plugin.settings.timeStepButtonLabel + "</button>");
								forwardButton.click(function()
								{
									var apiLocation = plugin.settings.apiRootLocation;
									if(parseInt(plugin.settings.apiId) > 100) {
									    apiLocation = plugin.settings.apiRootLocation2;
                                    }
									$.ajax({
										url: apiLocation + plugin.settings.apiId + "/reservecheck/",
										method: "POST",
										data: JSON.stringify({
											language: plugin.settings.language,
											numberOfPeople: plugin.reservation.numGuests,
											time: plugin.reservation.time,
											date: moment(plugin.reservation.date).format("YYYY.MM.DD"),
										...
										success: function() {
											plugin.displayStep(3)
										},
										error: function(jqXHR) {
											if (jqXHR.status == 400)
											{
												contentDiv.html("");
												contentDiv.append(plugin.getToleranceBreachMessage());
											}
											else {
												plugin.container.html("<h3>" + plugin.settings.apiErrorMessage + "</h3>");
											}
										}
									});
								});
								contentDiv.append(forwardButton);
							}
						}
					},
					{
						...
						setupFunction: function(stepContent, plugin) {
							$.validator.addMethod("isValueTelephone", function(value, element) {
								var re = new RegExp(/^[+()]*[0-9][- +()0-9]*$/);
								return this.optional(element) || (re.test(value));
							}, plugin.settings.telephoneValidationError);
							
							plugin.contactDetailsForm = $("<form id='contactDetails'></form>");
							plugin.contactDetailsForm.append($("<input placeholder='" + plugin.settings.nameLabel + "' type='text' id='name' name='name' required data-msg-required='" + plugin.settings.nameValidationError + "'>")
								.on('blur', function() { plugin.reservation.contactName = $(this).val(); }));
							plugin.contactDetailsForm.append($("<input placeholder='" + plugin.settings.emailLabel + "' type='email' id='email' name='email'>")
								.on('blur', function() { plugin.reservation.contactEmail = $(this).val(); }));
							var phoneInput = $("<input placeholder='" + plugin.settings.numberLabel + "' type='tel' id='phone' name='phone' required data-msg-required='" + plugin.settings.telephoneValidationError + "'>");
							plugin.contactDetailsForm.validate({
								rules : {
									phone : { isValueTelephone : true }
								}
							});
							phoneInput.on('blur', function() { plugin.reservation.contactPhone = $(this).val(); });
							plugin.contactDetailsForm.append(phoneInput)
							plugin.contactDetailsForm.append($("<textarea placeholder='" + plugin.settings.notesLabel + "' id='notes' name='notes'>")
								.on('blur', function() { plugin.reservation.notes = $(this).val(); }));
							plugin.contactDetailsForm.append($("<span>" + plugin.settings.marketingMessage + "</span>"));
							plugin.contactDetailsForm.append($("<input type='checkbox' id='marketingOpt' name='marketingOpt'>")
								.on('change', function() { plugin.reservation.marketingOpt = this.checked; }));
							plugin.reservation.marketingOpt = false;

							plugin.contactDetailsForm.find("[required]").each(function(index, element) {
								$(element).after("<span style='color:" + plugin.settings.highlightColor + "'>&nbsp;*</span>");
							});
							
							plugin.contactDetailsForm.validate();
							stepContent.append(plugin.contactDetailsForm);
							
							stepContent.append("<div style='clear:both'></div>");

							var backButton = $("<button class='left-bottom'>" + plugin.settings.backLabel + "</button>");
							backButton.click(function() {plugin.displayStep(2)});
							stepContent.append(backButton);

							var forwardButton = $("<button class='right-bottom'>" + plugin.settings.detailsStepButtonLabel + "</button>");
							forwardButton.click(function() {
								if (plugin.contactDetailsForm.valid()) {
									plugin.displayStep(4);
								}
							});
							stepContent.append(forwardButton);
						}
					},
					{
						headerTitle: "Display Message 4",
						setupFunction: function(stepContent, plugin) {
							stepContent.append("<span>Sending your reservation. Please wait ...</span>");
						},
						onLoad: function(contentDiv, plugin) {
							
							var reservationDate = {
								language: plugin.settings.language,
								name: plugin.reservation.contactName,
								telephone: plugin.reservation.contactPhone,
								notes: plugin.reservation.notes,
								email: plugin.reservation.contactEmail,
								numberOfPeople: plugin.reservation.numGuests,
								time: plugin.reservation.time,
								date: moment(plugin.reservation.date).format("YYYY.MM.DD"),
								marketingOpt: plugin.reservation.marketingOpt,
								token: plugin.settings.token
							};
							...
							$.ajax({
								type: "POST",
								url: apiLocation + plugin.settings.apiId + "/reserve/",
								...
								success: function(data) {
									if (data.accepted) {
										contentDiv.html(plugin.settings.confirmationMessage);
									}
									else {
				...
				contentHeight: "18em",
				apiId: 0,
				apiRootLocation: "http://book.api.epicuri.co.uk/booking/",
                apiRootLocation2: "https://api-prod.epicuri.co.uk/booking/",
				numGuestChoices: [ 1, 2, 3, 4, 5, 6, 7, 8 ],
				marketingMessage: "I would like to receive marketing emails from this restaurant.",
				nameValidationError: "Name Validation Error",
				telephoneValidationError: "Telephone Validation Error",
				apiErrorMessage: "Booking widget currently unavailable.",
		...
		$.extend(Plugin.prototype, {
				init: function () {
					$("<style type='text/css'>" + 
						".step-title{text-align:center;padding: 0.3em 0;font-size:0.8em;font-weight:bold;border-top:0.2em solid " + this.settings.highlightColor + 
							";border-bottom:0.2em solid " + this.settings.highlightColor + ";width:100%}" + 
						".epicuribooking-container a {color:" + this.settings.textColor + "}" +
						".epicuribooking-container{width:" + this.settings.width + ";height:" + this.settings.height + ";padding:0.5em;background-color:" + this.settings.backgroundColor + ";display:inline-block;color:" + this.settings.textColor + "}" + 
						".step-content{height:" + this.settings.contentHeight + ";}" + 
						".clickable-steps{margin:0.5em 0;text-align:center;background-color:" + this.settings.backgroundColor + ";color:" + this.settings.textColor + "}" + 
						"select.service-level,select.num-guests{margin-right:1em;}" +
						"button{background-color:" + this.settings.highlightColor + 
							";color:" + this.settings.textColor + ";border: 0;padding: 0.7em;" +
							"-webkit-border-radius: 3px;-moz-border-radius: 3px;border-radius: 3px;}" +
						".reservation-info {text-align:center;}" +
						".reservation-info label {margin-right:1em;}" +
						"button:hover{background-color:" + this.settings.highlightColorLight + "}" +
						"button.left-bottom{position:absolute;bottom:0;left:0}" +
						"button.right-bottom{position:absolute;bottom:0;right:0}" +
						//".pmu-button{background-color:" + this.settings.backgroundColorLight + ";color:" + this.settings.textColor + ";" +
						//	"margin: 0 1px 1px 0;}" +
						".pmu-today{background-color:" + this.settings.highlightColor + ";color:" + this.settings.textColor + ";}" +
						".pmu-selected{background-color:" + this.settings.highlightColorLight + ";color:" + this.settings.textColor + ";}" +
						".time-slot{padding:2px;display:inline-block;background-color:" + this.settings.highlightColor + 
							";color:" + this.settings.textColor + ";margin: 0 2px 2px 0;width: calc(25% - 6px);text-align:center;" + 
							"cursor:pointer;}" +
						".time-slot.active{background-color:" + this.settings.highlightColorLight + "}" +
						".party-size {}" +
						".party-size .hover-label {}" +
						"#contactDetails input[type='tel'],#contactDetails input[type='email'],#contactDetails input[type='text']," + 
							"#contactDetails textarea{display:inline-block;width:calc(100% - 2em);margin-top:5px;}" +
						"#contactDetails input[type='checkbox']{margin-left:1em;}" +
						"#contactDetails textarea{font-family:Arial;height:3em;}" +
						".footer{text-align:center;margin-top:1em;}" +
					"</style>").appendTo("head");

					this.container = $("<div class='epicuribooking-container'></div>");
					$(this.element).append(this.container);
					
					// Object for storing registration info
					this.reservation = {};
					var $this = this;
                    var apiLocation = this.settings.apiRootLocation;
                    if(parseInt(this.settings.apiId) > 100) {
                        apiLocation = this.settings.apiRootLocation2;
                    }
					$.ajax({
						type: "POST",
						url: apiLocation + this.settings.apiId + "/statics/",
						data: JSON.stringify({ language : this.settings.language }),
						cache: false,
						crossDomain: true,
						contentType: 'application/json',
						error: function(jqXHR, exception) {
							$this.container.append("<h3>" + $this.settings.apiErrorMessage + "</h3>");
						},
						success: function(data) {
							$this.settings = $.extend( {}, $this.settings, data );
							
							$this.settings.steps[0].headerTitle = $this.settings.dateStepMessage;
							$this.settings.steps[1].headerTitle = $this.settings.timeStepMessage;
							$this.settings.steps[2].headerTitle = $this.settings.detailsStepMessage;
							$this.settings.steps[3].headerTitle = $this.settings.confirmationStepMessage;
							
							$this.currentStep = $this.settings.startStep;
							
							if ($this.settings.title != null || $this.settings.title != "")
							{
								$this.container.append($("<h2 style='margin:0;text-align:center;'>" + $this.settings.title + "</h2>"));
							}
							
							$this.stepDisplayContainer = $("<div></div>");
							$this.stepDisplay = $("<h3 class='step-title'></h3>");
							$this.stepDisplayContainer.append($this.stepDisplay);
							
							$this.container.append($this.stepDisplayContainer);
							
							$this.stepLinks = $("<div class='clickable-steps'>Steps:&nbsp;</div>");
							
							$this.contentDiv = $("<div class='step-content'></div>");
							$this.container.append($this.contentDiv);
							
							for (var i = 1; i < 5; i++)
							{
								var step = $("<span data-step-num='" + i +
									"' style='margin:0 0.5em;width:1em;height:1em;display:inline-block;text-align:center;-webkit-border-radius: 1000px;" + 
									"-moz-border-radius: 1000px;border-radius: 1000px;color:" + $this.settings.highlightColor + "'>" + i + "</span>");
								if ($this.stepsClickable) {
									step.css("cursor", "pointer");
									step.click(function() {
										var thisStepNum = parseInt($(this).attr("data-step-num"));
										$this.displayStep(thisStepNum);
									});
								}
								$this.stepLinks.append(step);
								var stepContent = $("<div style='height:100%;position:relative;' data-step-num='" + i + "'></div>");
								stepContent.hide();
								
								var displayMessage = $("<span data-title-num='" + i + "'>"+ $this.settings.steps[i - 1].headerTitle + "</span>");
								displayMessage.hide();
								
								$this.stepDisplay.append(displayMessage);
								$this.settings.steps[i - 1].setupFunction(stepContent, $this);
								$this.contentDiv.append(stepContent);
							}
							
							$this.stepDisplayContainer.prepend($this.stepLinks);
							
							$this.displayStep($this.currentStep);
							
							if(! /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
								var footer = $("<div class='footer'></div>");
								if ($this.settings.entityEmail)
								{
									footer.append("<strong>Email:&nbsp;</strong>" + 
									"<span><a href='mailto:" + $this.settings.entityEmail + "'>" +
									$this.settings.entityEmail + "</a></span><br />");
								}
								if ($this.settings.entityNumber)
								{
									footer.append("<strong>Telephone:&nbsp;</strong><span>" + $this.settings.entityNumber + "</span>");
								}
								$this.container.append(footer);
							}
														
							console.log("Restaurant Plugin is Initialized");
						}
					});
				},
				/*
				 * Updates the interface to reflect the currently selected step
				 */
				renderCurrentStep: function() {
					// Update highlighted step
					var stepItems = $(this.stepLinks).children("span");
					stepItems.css("border", "0");
					
					$(this.stepLinks).children("span[data-step-num='" + this.currentStep + "']")
						.css("border", "0.2em solid " +  this.settings.highlightColor);
				},
				displayStep: function(stepNum) {
					if (stepNum && !isNaN(stepNum) && (stepNum < 1 || stepNum > 4)) {
						console.log("Error: Step number out of bounds. Choose a step between 1-4");
						return;
					}
					
					this.contentDiv.find("div[data-step-num]").hide();
					this.stepDisplay.find("span[data-title-num]").hide();
					
					this.currentStep = stepNum;
					
					this.contentDiv.find("div[data-step-num='" + stepNum + "']").show();
					this.stepDisplayContainer.find("span[data-title-num='" + stepNum + "']").show();
					
					if (this.settings.steps[stepNum - 1].onLoad != null) {
						var contentDiv = this.contentDiv.find("div[data-step-num='" + stepNum + "']");
						this.settings.steps[stepNum - 1].onLoad(contentDiv, this);
					}
					
					this.renderCurrentStep();
				},
				setReservationTime: function(time) {
					// Clear the CSS classes
					this.contentDiv.find(".time-slot.active").removeClass("active");
					// Set the CSS class of this button
					$(".time-slot:contains('" + time + "')").each(function(index, element) {
				...
				getReservationTimes: function() {
					...
					$.ajax({
						type: "POST",
						url: apiLocation + this.settings.apiId + "/timeslots/",
						data: JSON.stringify({ token: this.settings.token, date: moment(this.reservation.date).format("YYYY.MM.DD"), language: this.settings.language }),
						cache: false,
						crossDomain: true,
						async: false,
						contentType: 'application/json',
				...
				getToleranceBreachMessage: function() {
					var toleranceBreachMessage = $("<h3 style='text-align:center'>" + this.settings.toleranceBreachMessage + "</h3>");
					var $this = this;
					var chooseNewDateBtn = $("<button style='margin:1em 0'>Choose a new date</button>");
					chooseNewDateBtn.on("click", function() {
						$this.displayStep(1);
					});
					toleranceBreachMessage.append(chooseNewDateBtn);
					
					var chooseNewTimeBtn = $("<button>Choose a new time</button>");
					chooseNewTimeBtn.on("click", function() {
						$this.displayStep(2);
					});
					toleranceBreachMessage.append(chooseNewTimeBtn);
