...
public class MenuStructureReader {
    ...
    private final Map<String,HostTaxView> taxRatesToName;
    ...
    public MenuStructureReader(WebService api) {
        ...
        taxRatesToName = taxRates.stream().collect(Collectors.toMap(HostTaxView::getName, Function.identity()));
    ...
    private void read(File file) throws IOException {
        ...
        for (CSVRecord record : records) {
            ...
            String course = "";
            if(record.size() >= 9) {
                course = record.get(8).trim();
            }

            MenuItemView potentialDupe = isAlreadyInDB(name, price, description, taxType);
    ...
    private MenuItemView isAlreadyInDB(String name, double price, String description, String taxRateName) {
        for (MenuItemView item : existingMenuItemViews) {
            if (item.getName().equalsIgnoreCase(name)
                    && almostEqual(item.getPrice(), price, 0.001)
                    && item.getDescription().equalsIgnoreCase(description)
                    && item.getTaxTypeId().equals(taxRatesToName.get(taxRateName).getId())) {
                return item;
            }
        }
