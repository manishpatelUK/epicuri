...
public class ReportingServiceTest extends BaseReportingServiceTest {
    ...
    public void getCustomerDetailsReportLines() throws Exception {
        setUpBooking(1498690801000L, booking1, BookingType.RESERVATION, customer1);
        setUpBooking(1498690801000L, booking2, BookingType.RESERVATION, customer1);
        setUpBooking(1498690801000L, booking3, BookingType.RESERVATION, customer2);
    ...
    private void setUpBooking(long time, Booking booking, BookingType bookingType, Customer customer) {
        booking.setCustomerId(customer.getId());
        booking.setRestaurantId(restaurant1.getId());
        booking.setTargetTime(time);
        booking.setStaffId(staff1.getId());
        booking.setBookingType(bookingType);
        if(bookingType == BookingType.TAKEAWAY) {
    ...
    public void getAggregatedItemsReportLinesWithPLU() throws Exception {
        ...
        assertEquals(tax2.getName(), item.getTaxName());
        assertEquals("20.00%", item.getTaxRate());
    ...
    @Test
    public void getModifierDetailsReportLines() throws Exception {
        ReportingConstraints constraints = reportingService.createConstraints(token, "28-06-2017", "29-06-2017");
        List<ModifierDetailsReportLine> lines = reportingService.getModifierDetailsReportLines(constraints);
        assertEquals(1, lines.size());
        //todo
    }
    ...
    public void getReservationLines() throws Exception {
        setUpBookings(BookingType.RESERVATION);

        ReportingConstraints constraints = reportingService.createConstraints(token, "28-06-2017", "29-06-2017");
        List<ReservationLine> lines = reportingService.getReservationLines(constraints);
        assertEquals(3,lines.size());

        checkBookingLinesResult(lines.get(0));
    }

    private void checkBookingLinesResult(IBookingLine line) {
        assertEquals("2017-06-29 00:00", line.getTime());
        assertEquals(booking1.getName(), line.getName());
        assertEquals(line instanceof ReservationLine ? "10" : "0", line.getCovers());
        assertEquals(booking1.getTelephone(), line.getTelephone());
        assertEquals(booking1.getEmail(), line.getEmail());
        assertEquals(booking1.getNotes(), line.getNotes());
        assertEquals("ACCEPTED", line.getBookingState());
        assertEquals("ONLINE", line.getOrigin());
    }

    private void setUpBookings(BookingType bookingType) {
        setUpBooking(1498690801000L, booking1, bookingType, customer1);
        setUpBooking(1498690801000L + (1000*60*5), booking2, bookingType, customer1);
        setUpBooking(1498690801000L + (1000*60*10), booking3, bookingType, customer2);
    ...
    @Test
    public void getTakeawayLines() throws Exception {
        setUpBookings(BookingType.TAKEAWAY);

        ReportingConstraints constraints = reportingService.createConstraints(token, "28-06-2017", "29-06-2017");
        List<TakeawayLine> lines = reportingService.getTakeawayLines(constraints);
        assertEquals(3,lines.size());

        checkBookingLinesResult(lines.get(0));
        assertEquals(TakeawayType.DELIVERY.name(), lines.get(0).getTakeawayType());
    }
    ...
    public void testCreateCSVWrapper1() {
        ...
        for(String line : split) {
            assertEquals(line.endsWith(",") ? 9 : 10, line.split(",").length);
        }
        assertEquals(4, split.length);
        assertEquals("Date/Time,Booking State,Booking Via,Booked by Staff,Name,Covers,Phone,Email,Time Created,Notes", split[0]);
        String[] bits = split[1].split(",");
        assertEquals("2017-06-29 00:00", bits[0]);
        assertEquals("ACCEPTED", bits[1]);
        assertEquals("ONLINE", bits[2]);
        assertEquals("", bits[3]);
        assertEquals(booking1.getName(), bits[4]);
        assertEquals("10", bits[5]);
        assertEquals(booking1.getTelephone(), bits[6]);
        assertEquals(booking1.getEmail(), bits[7]);
        assertEquals(booking1.getNotes(), bits[9]);
    }
