...
public class BatchServiceTest extends BaseIT{
    ...
    @Test
    public void getBatches_takeaways() throws Exception {
        setUpOrders(session1, restaurant1, true, course1);
        printer1.setIp("192.1.1.1");
        printerRepository.save(printer1);

        Batch batch = new Batch();
        batch.setSessionId(session1.getId());
        batch.getOrderIds().add(order1.getId());
        batch.setSessionType(SessionType.TAKEAWAY);
        batch.setPrinterId(printer1.getId());
        batch.setCreationTime(System.currentTimeMillis());
        batch.setIntendedPrintTime(System.currentTimeMillis()+1);
        batch.setAwaitingImmediatePrint(false);
        batch = batchRepository.insert(batch);
        ...
        booking1.setCreatedTime(System.currentTimeMillis());
        booking1.setTargetTime(session1.getStartTime());
        bookingRepository.save(booking1);

        List<HostBatchView> hostBatchViews = batchService.getHostBatchViews(restaurant1.getId());
        assertEquals(1, hostBatchViews.size());
        assertEquals(batch.getId(), hostBatchViews.get(0).getId());
    }
