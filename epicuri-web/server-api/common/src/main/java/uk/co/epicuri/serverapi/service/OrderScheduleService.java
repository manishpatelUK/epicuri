...
public class OrderScheduleService {
    ...
    public List<PrinterTicketsResponse> getScheduleList(Restaurant restaurant, String printerId, boolean aggregateOnSession) {
        ...
        sessions.removeIf(Session::isLinked);
        ...
        filterForPrinter(printerId, restaurant.getDefaultTakeawayPrinterId(), orders, items, sessions, printerMap);
        ...
        sessions.stream().filter(s -> ordersKeys.contains(s.getId())).forEach(s -> {
            List<Order> sessionOrders = orders.get(s.getId());
            if(aggregateOnSession) {
                PrinterTicketsResponse response = getPrinterTicketsAggregatedBySession(restaurant, sessionOrders, items, courseMap, s,batchesBySession.get(s.getId()));
                responses.add(response);
            } else {
                List<PrinterTicketsResponse> response = getPrinterTicketsAggregatedByBatch(restaurant, sessionOrders, items, courseMap, s,batchesBySession.get(s.getId()));
    ...
    public List<PrinterTicketsResponse> getPrinterTicketsAggregatedByBatch(Restaurant restaurant, List<Order> orders, Map<String, MenuItem> items, Map<String, Course> courseMap, Session session, List<Batch> batches) {
        ...
        if(batches == null || batches.size() == 0) {
            return Collections.singletonList(getPrinterTicketsAggregatedBySession(restaurant, orders, items, courseMap, session, batches));
        }
        ...
        for(Batch batch : batches) {
            ...
            PrinterTicketsResponse ticket = getPrinterTicketsAggregatedBySession(restaurant, ordersForBatch, items, courseMap, session, batches);
            if(session.getSessionType() != SessionType.TAKEAWAY) {
                ...
                } else if (session.getClosedTime() != null) {
    ...
    public PrinterTicketsResponse getPrinterTicketsAggregatedBySession(Restaurant restaurant, List<Order> orders, Map<String, MenuItem> items, Map<String, Course> courseMap, Session session, List<Batch> batches) {
    ...
    private void filterForPrinter(String printerId, String takeawayPrinterId, Map<String, List<Order>> orders, Map<String, MenuItem> items, List<Session> sessions, Map<String,Printer> printers) {
        ...
        orders.forEach((k,v) -> filterForPrinter(printerId, takeawayPrinterId, items, v, sessionMap.get(k), printers));
    }

    private void filterForPrinter(String printerId, String takeawayPrinterId, Map<String, MenuItem> items, List<Order> orders, Session session, Map<String,Printer> printers) {
        if(session.getSessionType() != SessionType.TAKEAWAY) {
            ...
            while (iterator.hasNext()) {
                ...
                if (items.containsKey(o.getMenuItemId())) {
                    ...
                    if(designatedPrinterId == null) {
                        continue;
                    }
                    if(printers.get(designatedPrinterId) != null) {
                        if(printers.get(designatedPrinterId).getDuplicateTo() != null && printers.get(designatedPrinterId).getDuplicateTo().contains(printerId)) {
                            continue;
                        }
                    }
                    if (!designatedPrinterId.equals(printerId)) {
                        iterator.remove();
                    }
                } else {
                    iterator.remove();
