...
public class SessionCalculationService {
    ...
    public Map<CalculationKey,Number> calculateValues(Session session, List<Order> orders) {
        ...
        int nonDeferredAdjustmentValue = 0;
        boolean sessionDeferred = session.isDeferred();
        Adjustment deferralAdjustment = sessionDeferred ? session.getAdjustments().stream().filter(a -> a.getAdjustmentType().getName().equals(RestaurantConstants.DEFER_ADJUSTMENT)).findFirst().orElse(null) : null;
        for(Adjustment adjustment : session.getAdjustments()) {
            ...
            if(type == AdjustmentTypeType.DISCOUNT) {
                ...
                if (adjustment.getNumericalType() == NumericalAdjustmentType.ABSOLUTE) {
                    ...
                    discountTotal += adj;
                    runningTotal -= adj;
                    if(sessionDeferred && deferralAdjustment != null && !deferralAdjustment.getId().equals(adjustment.getId())) {
                        nonDeferredAdjustmentValue += adj;
                    }
                }
                else if (adjustment.getNumericalType() == NumericalAdjustmentType.PERCENTAGE) {
                    if(adjustment.getApplicableToItems() == ItemType.ALL) {
                        if (runningTotal > 0) {
                            if (adj >= 1000) {
                                discountTotal += runningTotal;
                                runningTotal = 0;
                                if (sessionDeferred && deferralAdjustment != null && !deferralAdjustment.getId().equals(adjustment.getId())) {
                                    nonDeferredAdjustmentValue += runningTotal;
                                }
                            } else {
                                ...
                                if (sessionDeferred && deferralAdjustment != null && !deferralAdjustment.getId().equals(adjustment.getId())) {
                                    nonDeferredAdjustmentValue += thisDiscount;
                                }
                            }
                        }
                    } else {
                        int totalFromType = summary.getItemTypeTotal().get(adjustment.getApplicableToItems());
                        if (runningTotal > 0) {
                            int thisDiscount = discountOperatingValueByPercentage(adjustment.getValue(), totalFromType);
                            discountTotal += thisDiscount;
                            runningTotal -= thisDiscount;
                            if (sessionDeferred && deferralAdjustment != null && !deferralAdjustment.getId().equals(adjustment.getId())) {
                                nonDeferredAdjustmentValue += thisDiscount;
        ...
        calculatedValues.put(CalculationKey.NON_DEFERRED_DISCOUNTS_VALUE, nonDeferredAdjustmentValue);
        ...
        if(session.getTipPercentage() != null) {
            ...
            BigDecimal calculatedTipValueBeforeDiscount = BigDecimal.valueOf(session.getTipPercentage()).scaleByPowerOfTen(-2).multiply(BigDecimal.valueOf(subTotal));
            calculatedValues.put(CalculationKey.TIP_BEFORE_DISCOUNT, MoneyService.toPenniesRoundNearest(calculatedTipValueBeforeDiscount));
        }
        ...
        if(session.getOriginalBooking() != null
            ...
            remainingTotal += calculatedDeliveryCost;
            BigDecimal deliveryVAT = calculateVATonDelivery(calculatedDeliveryCost, orders);
            calculatedValues.put(CalculationKey.VAT_DELIVERY, deliveryVAT.intValue());
            vatTotal = vatTotal.add(deliveryVAT);
        }
        ...
        calculatedValues.put(CalculationKey.VAT_TOTAL_BEFORE_ADJUSTMENTS,
                          calculatedValues.getOrDefault(CalculationKey.VAT_TOTAL_BEFORE_ADJUSTMENTS, 0).intValue()
                        + calculatedValues.getOrDefault(CalculationKey.VAT_DELIVERY, 0).intValue());
    ...
    private static BigDecimal calculateVATonDelivery(int deliveryCost, List<Order> orders) {
        int maxRate = 0;
        for(Order order : orders) {
            TaxRate taxRate = order.getTaxRate();
            if(taxRate != null) {
                maxRate = Math.max(maxRate, taxRate.getRate());
            }
        }

        BigDecimal bdDeliveryCost = BigDecimal.valueOf(deliveryCost);
        double actualTaxRate = TaxRate.getRateAsDouble(maxRate);
        return bdDeliveryCost.subtract(bdDeliveryCost.divide(BigDecimal.valueOf(1 + actualTaxRate), BigDecimal.ROUND_HALF_UP));
    }
