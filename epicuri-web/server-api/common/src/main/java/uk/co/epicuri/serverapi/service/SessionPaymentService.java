...
public class SessionPaymentService {
    ...
    public List<Adjustment> processPayment(Session session, CreditCardData creditCardData, int total, int gratuity, boolean capture, boolean isCustomer) {
        ...
        AdjustmentType adjustmentPaymentType = getPaymentAdjustmentType(restaurant, StripeConstants.STRIPE_PAYMENT_TYPE);
        AdjustmentType adjustmentGratuityType = getGratuityAdjustmentType(restaurant, StripeConstants.STRIPE_GRATUITY_TYPE);
    ...
    public List<Adjustment> processCaymanPayment(Session session, CaymanGatewayPayload payload) {
        Map<CalculationKey, Number> calculationKeyNumberMap = sessionCalculationService.calculateValues(session);
        List<Adjustment> adjustments = new ArrayList<>();
        Restaurant restaurant = masterDataService.getRestaurant(session.getRestaurantId());
        AdjustmentType adjustmentPaymentType = getPaymentAdjustmentType(restaurant, CaymanGatewayConstants.CAYMAN_PAYMENT_TYPE);
        if(adjustmentPaymentType == null) {
            return adjustments;
        }
        ...
        mainAdjustment.setAdjustmentType(adjustmentPaymentType);
        mainAdjustment.setNumericalType(NumericalAdjustmentType.ABSOLUTE);
        mainAdjustment.setValue(calculationKeyNumberMap.get(CalculationKey.SESSION_TOTAL).intValue());
        mainAdjustment.getSpecialAdjustmentData().put(CaymanGatewayConstants.PAYMENT_KEY, payload);
        mainAdjustment.setCreated(System.currentTimeMillis());
        adjustments.add(mainAdjustment);
    ...
    public AdjustmentType getPaymentAdjustmentType(Restaurant restaurant, String paymentName) {
        List<AdjustmentType> adjustmentTypes = masterDataService.getAdjustmentTypes(restaurant.getAdjustmentTypes());
        return adjustmentTypes.stream().filter(a -> a.getName().equals(paymentName)).findFirst().orElse(null);
    }

    public AdjustmentType getGratuityAdjustmentType(Restaurant restaurant, String paymentName) {
        List<AdjustmentType> adjustmentTypes = masterDataService.getAdjustmentTypes(restaurant.getAdjustmentTypes());
        return adjustmentTypes.stream().filter(a -> a.getName().equals(paymentName)).findFirst().orElse(null);
    }
}
