...
public class TimeUtil {
    ...
    private static final DateTimeFormatter CUSTOMER_FACING_DATETIME_FORMATTER = DateTimeFormatter.ofPattern("EEE dd MMM HH:mm");
    ...
    public static String toString(long utcTime, String restaurantZoneId) {
        return CUSTOMER_FACING_DATETIME_FORMATTER.format(getRestaurantTime(utcTime, restaurantZoneId));
    }
    ...
    public static ZonedDateTime getLastMidnight(String restaurantZoneId) {
        return getLastMidnight(restaurantZoneId, 0);
    }

    public static ZonedDateTime getLastMidnight(String restaurantZoneId, int dayOffset) {
        return getLastMidnight(restaurantZoneId, dayOffset, System.currentTimeMillis());
    }

    public static ZonedDateTime getLastMidnight(String restaurantZoneId, int dayOffset, long fromTime) {
        return getRestaurantTime(fromTime + (dayOffset * 86400000L), restaurantZoneId).truncatedTo(ChronoUnit.DAYS);
    }

    public static long toEpochMillisToday(String hhmm, String restaurantZoneId) {
        return toEpochMillisDayOffsetFromToday(hhmm, restaurantZoneId, 0);
    }

    public static long toEpochMillisDayOffsetFromToday(String hhmm, String restaurantZoneId, int dayOffset) {
        ZonedDateTime lastMidnight = getLastMidnight(restaurantZoneId, dayOffset);
