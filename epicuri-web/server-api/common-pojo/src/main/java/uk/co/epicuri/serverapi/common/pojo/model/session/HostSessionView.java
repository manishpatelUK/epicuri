...
public class HostSessionView {
    ...
    @JsonProperty("TipBeforeDeferment")
    private double tipBeforeDeferment;

    @JsonProperty("TotalBeforeDeferment")
    private double totalBeforeDeferment;
    ...
    public HostSessionView(Session session,
        ...
        if(session.getSessionType() == SessionType.TAKEAWAY) {
            if(calculatedValues.get(CalculationKey.DELIVERY_TOTAL) != null) {
                this.deliveryCost = MoneyService.toMoneyRoundNearest(calculatedValues.get(CalculationKey.DELIVERY_TOTAL).intValue());
            } else {
    ...
    private void setCommonProperties(Session session, Map<CalculationKey,Number> calculatedValues, Map<String,Staff> allStaff, boolean isPaid) {
        ...
        if(session.isDeferred()) {
            tipBeforeDeferment = MoneyService.toMoneyRoundNearest(calculatedValues.getOrDefault(CalculationKey.TIP_BEFORE_DISCOUNT, 0).intValue());
            double totalBeforeAdjustments = MoneyService.toMoneyRoundNearest(calculatedValues.get(CalculationKey.TOTAL_BEFORE_ADJUSTMENTS).intValue());
            double nonDeferralAdjustments = MoneyService.toMoneyRoundNearest(calculatedValues.getOrDefault(CalculationKey.NON_DEFERRED_DISCOUNTS_VALUE, 0).intValue());
            totalBeforeDeferment = tipBeforeDeferment + (totalBeforeAdjustments - nonDeferralAdjustments);
        }
        linkedTo = session.getLinkedSession();
    }
    ...
    public double getTipBeforeDeferment() {
        return tipBeforeDeferment;
    }

    public void setTipBeforeDeferment(double tipBeforeDeferment) {
        this.tipBeforeDeferment = tipBeforeDeferment;
    }

    public double getTotalBeforeDeferment() {
        return totalBeforeDeferment;
    }

    public void setTotalBeforeDeferment(double totalBeforeDeferment) {
        this.totalBeforeDeferment = totalBeforeDeferment;
    }
