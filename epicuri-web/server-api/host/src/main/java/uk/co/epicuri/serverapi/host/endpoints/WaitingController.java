...
public class WaitingController {
    ...
    private Tuple<Session,OrderResponse> createAdhocSession(String token, WaitingPartyOrderPayload payload, boolean willAttemptImmediatePrint, boolean orderPrintsRequired) throws IllegalArgumentException{
        ...
        for(WaitingPartyOrderPayload.OrderPayload o : payload.getOrder()) {
            ...
            if(o.getQuantity() > 0
                ...
                Order order = new Order(session, item, taxRateMap.get(item.getTaxTypeId()), o, modifiersWithTaxes, price, staffId);
        ...
        liveDataService.insertOrders(session, allMenuItems, orders, session.getSessionType() != SessionType.REFUND, orderPrintsRequired);
        ...
        List<HostBatchView> list = batchService.getHostBatchViews(restaurantId,
                ...
                session.getOriginalBookingId() == null ? new HashMap<>() : Collections.singletonMap(session.getOriginalBookingId(), session.getOriginalBooking()),
