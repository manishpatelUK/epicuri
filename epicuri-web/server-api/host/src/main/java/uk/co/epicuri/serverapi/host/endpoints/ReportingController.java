...
public class ReportingController {
    ...
    @HostAuthRequired
    @ResponseBody
    @RequestMapping(value = "/modifierSales{.csv}", method = RequestMethod.GET, produces = TEXT_CSV)
    public CSVWrapper getModifierSales(@RequestHeader(Params.AUTHORIZATION) String token,
                                     @RequestParam("start") String start, @RequestParam("end") String end) {
        ReportingConstraints reportingConstraints = reportingService.createConstraints(token, start, end);
        List<ModifierDetailsReportLine> lines = reportingService.getModifierDetailsReportLines(reportingConstraints);
        LOGGER.trace("itemDetails -> {} rows", lines.size());

        return reportingService.createCsvWrapper(lines, ModifierDetailsReportLine.class, "Modifier_Sales.csv");
    }
    ...
    @HostAuthRequired
    @ResponseBody
    @RequestMapping(value = "/takeaways{.csv}", method = RequestMethod.GET, produces = TEXT_CSV)
    public CSVWrapper getTakeaways(@RequestHeader(Params.AUTHORIZATION) String token,
                                      @RequestParam("start") String start, @RequestParam("end") String end) {
        ReportingConstraints reportingConstraints = reportingService.createConstraints(token, start, end);
        List<TakeawayLine> lines = reportingService.getTakeawayLines(reportingConstraints);
        LOGGER.trace("takeaways -> {} rows", lines.size());

        return reportingService.createCsvWrapper(lines, TakeawayLine.class, "Takeaways.csv");
    }
