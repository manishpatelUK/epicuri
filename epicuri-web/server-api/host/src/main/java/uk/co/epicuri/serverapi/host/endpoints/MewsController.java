...
public class MewsController {
    ...
    public ResponseEntity<?> postAdjustment(@RequestHeader(Params.AUTHORIZATION) String token,
        ...
        try {
            String[] accountAndProductList = kvData.getSecret() == null ? new String[]{"","","",""} : kvData.getSecret().split(","); // bit of a hack
            Map<ItemType,String> accountingCategories = new HashMap<>();
            accountingCategories.put(ItemType.FOOD, accountAndProductList[0]);
            accountingCategories.put(ItemType.DRINK, accountAndProductList[1]);
            accountingCategories.put(ItemType.OTHER, accountAndProductList[2]);

            String chargeId = mewsService.charge(kvData.getHost(),
                                    kvData.getToken(),
                                    kvData.getKey(),
                                    accountingCategories,
                                    accountAndProductList[3],
