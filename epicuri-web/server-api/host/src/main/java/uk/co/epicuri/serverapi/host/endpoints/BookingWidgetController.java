...
import uk.co.epicuri.serverapi.common.pojo.booking.*;
...
public class BookingWidgetController {
    ...
    public ResponseEntity<BookingRequest> postReserve(@PathVariable("id") String id,
        ...
        if(StringUtils.isBlank(request.getName())) {
            ...
            error.setToleranceBreachMessage("Name is required - please amend");
            ResponseEntity.BodyBuilder bodyBuilder = ResponseEntity.badRequest();
            bodyBuilder.body(error);
            return bodyBuilder.build();
        }

        if(StringUtils.isBlank(request.getTelephone())) {
            BookingError error = new BookingError();
            error.setToleranceBreachMessage("Telephone is required - please amend");
            ResponseEntity.BodyBuilder bodyBuilder = ResponseEntity.badRequest();
            bodyBuilder.body(error);
            return bodyBuilder.build();
        }
