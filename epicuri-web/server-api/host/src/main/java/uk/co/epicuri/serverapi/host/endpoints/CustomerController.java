...
public class CustomerController {
    ...
    @HostAuthRequired
    @RequestMapping(path = "interactions", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<HostCustomerViewBasic>> getDeferredPaymentCustomers(@RequestHeader(Params.AUTHORIZATION) String token) {
        List<CustomerInteraction> customerInteractions = customerService.getCustomerInteractions(authenticationService.getRestaurantId(token));
        customerInteractions.removeIf(c -> c.getCustomerId() == null);
        List<Customer> customers = customerService.getCustomers(customerInteractions.stream().map(CustomerInteraction::getCustomerId).collect(Collectors.toList()));
        customers.sort((o1, o2) -> {
            String name1 = Customer.determineName(o1);
            String name2 = Customer.determineName(o2);
            return name1.compareTo(name2);
        });
        return ResponseEntity.ok(customers.stream().map(HostCustomerViewBasic::new).collect(Collectors.toList()));
    }
}
