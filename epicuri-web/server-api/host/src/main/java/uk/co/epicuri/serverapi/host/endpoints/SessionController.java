...
public class SessionController {
    ...
    public ResponseEntity<List<HostDeferredSession>> getDeferred(@RequestHeader(Params.AUTHORIZATION) String token) {
        String restaurantId = authenticationService.getRestaurantId(token);
        List<CustomerInteractionDeferredSession> deferredSessions = liveDataService.getDeferredSessions(restaurantId, false);
        if(deferredSessions.size() == 0) {
            return ResponseEntity.ok(new ArrayList<>());
        }
        Map<String,Customer> customerIds = customerService.getCustomerByIds(deferredSessions.stream().map(CustomerInteractionDeferredSession::getCustomerId).distinct().collect(Collectors.toList()))
                .stream().collect(Collectors.toMap(Customer::getId, Function.identity()));
        List<HostDeferredSession> view = new ArrayList<>();
        for(CustomerInteractionDeferredSession deferredSession : deferredSessions) {
            Session realSession = sessionService.findSession(deferredSession.getSessionId());
            // remove the discount on the session
            realSession.getAdjustments().removeIf(SessionService::isDeferredAdjustment);
            List<Order> orders = liveDataService.findOrders(realSession);
            Map<CalculationKey,Number> calculations = sessionCalculationService.calculateValues(realSession, orders);
            view.add(new HostDeferredSession(deferredSession, customerIds.get(deferredSession.getCustomerId()), calculations));
        }
        view.sort((o1, o2) -> Long.compare(o2.getCreationTime(), o1.getCreationTime()));
        return ResponseEntity.ok(view);
    }

    @HostAuthRequired
    @RequestMapping(value = "/{id}/defer", method = RequestMethod.PUT)
    public ResponseEntity<?> putDeferPayment(@RequestHeader(Params.AUTHORIZATION) String token,
                                             @PathVariable("id") String id,
                                             @RequestBody HostCustomerViewBasic customer) {
        String restaurantId = authenticationService.getRestaurantId(token);
        AdjustmentType deferredPaymentType = masterDataService.getDeferredPaymentType(restaurantId);
        if(deferredPaymentType == null) {
            return ResponseEntity.badRequest().body(new StringMessage("This venue is not set up to accept Deferred payments. Is the Deferred Discount type added to configuration?"));
        }

        if(sessionService.defermentExists(restaurantId, id)) {
            return ResponseEntity.badRequest().body(new StringMessage("This session is already deferred"));
        }
        ...
        if(StringUtils.isNotBlank(customer.getId()) && !customerService.exists(customer.getId())) {
            return ResponseEntity.badRequest().body(new StringMessage("Customer data has not been provided"));
        }

        if(StringUtils.isBlank(customer.getId()) && customer.getPhoneNumber().length() <= 6) {
            return ResponseEntity.badRequest().body(new StringMessage("Invalid phone number"));
        }

        if(StringUtils.isBlank(customer.getId())) {
            // create a customer
            Customer epicuriCustomer = customerService.getCustomerByPhone(customer.getPhoneNumber());
            if(epicuriCustomer == null) {
                ...
                epicuriCustomer = customerService.createCustomer(country.getAcronym(), customer.getPhoneNumber(), "", customer.getName().getFirstName(), customer.getName().getLastName(), false);
            }
            customer.setId(epicuriCustomer.getId());
        }

        sessionService.deferPayment(id, deferredPaymentType, authenticationService.getStaffId(token), customer);
        return ResponseEntity.ok().build();
    }

    @HostAuthRequired
    @RequestMapping(value = "/{id}/settleDeferred", method = RequestMethod.PUT)
    public ResponseEntity<?> putSettleDeferred(@RequestHeader(Params.AUTHORIZATION) String token,
                                               @PathVariable("id") String id,
                                               @RequestBody AdjustmentRequest request) {
        String staffId = authenticationService.getStaffId(token);

        try {
            sessionService.settleDeferredSession(id, staffId, request);
        } catch (IllegalStateException ex) {
            return ResponseEntity.badRequest().body(new StringMessage(ex.getMessage()));
        }
    ...
    @HostAuthRequired
    @RequestMapping(value = "/{id}/copyDeferred", method = RequestMethod.PUT)
    public ResponseEntity<?> putCopyDeferred(@RequestHeader(Params.AUTHORIZATION) String token,
                                             @PathVariable("id") String id) {
        String restaurantId = authenticationService.getRestaurantId(token);

        if(!sessionService.defermentExists(restaurantId, id)) {
            return ResponseEntity.badRequest().body(new StringMessage("This session is not deferred - cannot settle."));
        }

        try {
            Session session = sessionService.copyDeferredSessionAndOrders(id).getMiddle();
            return ResponseEntity.ok(new IdPojo(session.getId()));
        } catch (IllegalStateException ex) {
            return ResponseEntity.badRequest().body(new StringMessage(ex.getMessage()));
        }
    }
    ...
    @HostAuthRequired
    @RequestMapping(value = "/{id}/voidDeferred", method = RequestMethod.PUT)
    public ResponseEntity<?> voidDeferred(@RequestHeader(Params.AUTHORIZATION) String token,
                                             @PathVariable("id") String id) {
        ...
        try {
            Session session = sessionService.voidDeferredSessionAndOrders(id, authenticationService.getStaffId(token));
            return ResponseEntity.ok(new IdPojo(session.getId()));
        } catch (IllegalStateException ex) {
            return ResponseEntity.badRequest().body(new StringMessage(ex.getMessage()));
        }
