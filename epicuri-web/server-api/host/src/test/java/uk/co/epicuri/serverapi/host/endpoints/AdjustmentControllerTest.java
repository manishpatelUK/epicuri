...
public class AdjustmentControllerTest extends SessionSetupBaseIT {
    ...
    @Test
    public void postAdjustmentWithFoodOnlyDiscount() throws Exception {
        String token = getTokenForStaff(staff1);
        ...
        adjustmentRequest.setAdjustmentTypeId(adjustmentType1.getId());
        adjustmentRequest.setNumericalTypeId(NumericalAdjustmentType.toClientId(NumericalAdjustmentType.PERCENTAGE));
        adjustmentRequest.setValue(100);
        adjustmentRequest.setItemType(ItemType.FOOD.getRequestName());
        ...
        assertEquals(100, calculationKeyNumberMap.get(CalculationKey.TOTAL));
    }
    ...
    @Test
    public void postAdjustmentDeferred() {
        String token = getTokenForStaff(staff1);
        session1.setLinkedSession(Lists.newArrayList(session2.getId()));
        sessionRepository.save(session1);
        CustomerInteractionDeferredSession interaction = new CustomerInteractionDeferredSession(customer1.getId(), restaurant1.getId(), session2.getId(), staff1.getId());
        interaction = customerInteractionRepository.save(interaction);
        ...
        Response response = given()
                ...
                .post("Adjustment");
        assertEquals(HttpStatus.OK.value(), response.getStatusCode());
        assertTrue(((CustomerInteractionDeferredSession)customerInteractionRepository.findOne(interaction.getId())).isPaid());
        Session session = sessionRepository.findOne(session1.getId());
        assertEquals(1,session.getAdjustments().size());
        testEquals(session.getAdjustments().get(0), adjustmentRequest, adjustmentType2, NumericalAdjustmentType.ABSOLUTE);
    }
