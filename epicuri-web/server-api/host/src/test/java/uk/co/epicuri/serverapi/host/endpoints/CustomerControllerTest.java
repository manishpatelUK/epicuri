...
public class CustomerControllerTest extends BaseIT {
    ...
    @Test
    public void testGetDeferredPaymentCustomers() {
        customerInteractionRepository.insert(new CustomerInteractionDeferredSession(customer1.getId(), restaurant1.getId(), session1.getId(), staff1.getId()));
        String token = getTokenForStaff(staff1);
        ...
        assertEquals(HttpStatus.OK.value(), response.getStatusCode());
        List<HostCustomerViewBasic> hostCustomerViews = Arrays.asList(response.getBody().as(HostCustomerViewBasic[].class, ObjectMapperType.JACKSON_2));
        assertEquals(1, hostCustomerViews.size());
        HostCustomerViewBasic hostCustomerViewBasic = hostCustomerViews.get(0);
        assertEquals(customer1.getId(), hostCustomerViewBasic.getId());
        assertEquals("(44)0" + customer1.getPhoneNumber(), hostCustomerViewBasic.getPhoneNumber());
    }
}