...
public class KitchenControllerTest extends SessionSetupBaseIT {
    ...
    @Test
    public void getOrdersAllPrintersAggregateBySession_seated() throws Exception {
        ...
        checkResponse(token, "aggregateBySession", "true");
    }

    @Test
    public void getOrdersAllPrintersAggregateBySession_takeaway() throws Exception {
        String token = getTokenForStaff(staff1);

        setUpOrders(session1, restaurant1, true, course1);
        session1.setSessionType(SessionType.TAKEAWAY);
        sessionRepository.save(session1);
        Booking originalBooking = session1.getOriginalBooking();
        originalBooking.setTargetTime(session1.getStartTime() + (1000*60));
        bookingRepository.save(originalBooking);

        checkResponse(token, "aggregateBySession", "true");
    }

    private void checkResponse(String token, String aggregate, String flag) {
        Response response = given()
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .header(Params.AUTHORIZATION, token)
                .queryParam(aggregate, flag)
                .get("Kitchen/Orders");
    ...
    @Test
    public void getOrdersAllPrintersAggregateBySessionWhenLinked() throws Exception {
        String token = getTokenForStaff(staff1);

        setUpOrders(session1, restaurant1, true, course1);
        session1.setLinkedSession(Lists.newArrayList("foobar"));
        sessionRepository.save(session1);
    ...
    @Test
    public void getOrdersAllPrintersAggregateByBatchWhenNoBatches_seated() throws Exception {
        String token = getTokenForStaff(staff1);

        setUpOrders(session1, restaurant1, true, course1);
        session1.setSessionType(SessionType.SEATED);
        sessionRepository.save(session1);
        checkResponse(token, "aggregateBySession", "false");
    }

    @Test
    public void getOrdersAllPrintersAggregateByBatchWhenNoBatches_takeaway() throws Exception {
        String token = getTokenForStaff(staff1);
    ...
    public void getOrdersAllPrinters3() throws Exception {
        ...
        sessionRepository.save(session3);
        session4.setSessionType(SessionType.SEATED);
        sessionRepository.save(session4);
    ...
    private void assertEqual(PrinterTicketsResponse response, Session session, boolean isAggregatedSession) {
        ...
        if(isAggregatedSession) {
            ...
            assertEquals("12:01, 01/06/2017", response.getSessionTime());
            /*if(session.getSessionType() == SessionType.TAKEAWAY) {
                assertEquals("12:02, 01/06/2017", response.getSessionTime());
            } else {
                assertEquals("12:01, 01/06/2017", response.getSessionTime());
            }*/
        }
