...
public class SessionControllerTest extends SessionSetupBaseIT {
    ...
    @Test
    public void testPutSessionDeferred1() throws Exception {
        String token = getTokenForStaff(staff1);
        session1.setRestaurantId(restaurant1.getId());
        sessionRepository.save(session1);

        HostCustomerViewBasic hostCustomerViewBasic = new HostCustomerViewBasic();
        ...
        Response response = given()
                ...
                .header(Params.AUTHORIZATION, token)
                .pathParam("id", session1.getId())
                .body(hostCustomerViewBasic)
                .put("Session/{id}/defer");
        assertEquals(HttpStatus.BAD_REQUEST.value(), response.getStatusCode());
        ...
        hostCustomerViewBasic.setId("foomanchu");
        ...
        hostCustomerViewBasic.setId(customer1.getId());
        ...
        assertSessionIsDeferred(sessionRepository.findOne(session1.getId()), customer1);
    ...
    private void assertSessionIsDeferred(Session session, Customer customer) {
        assertNotNull(session.getClosedBy());
        assertNotNull(session.getClosedTime());
        assertEquals(1, session.getAdjustments().size());
        assertEquals(adjustmentType1, session.getAdjustments().get(0).getAdjustmentType());
        assertEquals(1000, session.getAdjustments().get(0).getValue());
        CustomerInteractionDeferredSession customerInteraction = (CustomerInteractionDeferredSession)customerInteractionRepository.findAll().get(0);
        assertNotNull(customerInteraction);
        assertEquals(customer.getId(), customerInteraction.getCustomerId());
        assertEquals(session1.getRestaurantId(), customerInteraction.getRestaurantId());
        assertEquals(session1.getId(), customerInteraction.getSessionId());
        assertEquals(staff1.getId(), customerInteraction.getStaffId());
    }

    @Test
    public void testPutSessionDeferred2() throws Exception {
        String token = getTokenForStaff(staff1);
        session1.setRestaurantId(restaurant1.getId());
        sessionRepository.save(session1);
        adjustmentType1.setName(RestaurantConstants.DEFER_ADJUSTMENT);
        ...
        hostCustomerViewBasic.setName(name);
        hostCustomerViewBasic.setPhoneNumber("077777777777");

        doDefer(token, hostCustomerViewBasic);

        Customer customer = customerRepository.findAll().stream().filter(c -> c.getFirstName().equals("foo")).findFirst().orElse(null);
        assertNotNull(customer);
        assertSessionIsDeferred(sessionRepository.findOne(session1.getId()), customer);
    }

    @Test
    public void testDefermentNumbers() {
        String token = getTokenForStaff(staff1);
        session1.setTipPercentage(10D);
        sessionRepository.save(session1);
        ...
        // temporarily take out the deferment to get totals numbers
        session1 = sessionRepository.findOne(session1.getId());
        Adjustment deferAdj = session1.getAdjustments().get(0);
        ...
        Response getSessionResponse = given()
                ...
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .header(Params.AUTHORIZATION, token)
                .get("Session/"+session1.getId());
        assertEquals(HttpStatus.OK.value(), getSessionResponse.getStatusCode());
        HostSessionView session = getSessionResponse.getBody().as(HostSessionView.class);
        double total = session.getTotal();
        double tip = session.getTips();

        // put the deferment back in
        session1.getAdjustments().add(deferAdj);
        sessionRepository.save(session1);

        getSessionResponse = given()
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .header(Params.AUTHORIZATION, token)
                .get("Session/"+session1.getId());
        assertEquals(HttpStatus.OK.value(), getSessionResponse.getStatusCode());
        session = getSessionResponse.getBody().as(HostSessionView.class);

        assertEquals(total, session.getTotalBeforeDeferment(), 0.0001);
        assertEquals(tip, session.getTipBeforeDeferment(), 0.0001);
    }

    @Test
    public void testPutSessionDeferred3() throws Exception {
        String token = getTokenForStaff(staff1);
        int size = customerRepository.findAll().size();
        deferSessionExistingCustomerNoId(token);
        assertEquals(size, customerRepository.findAll().size());
    }

    private void deferSession(String token) {
        setUpDeferment();
    ...
    private void doDefer(String token, HostCustomerViewBasic hostCustomerViewBasic) {
        Response deferResponse = given()
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .header(Params.AUTHORIZATION, token)
                .pathParam("id", session1.getId())
                .body(hostCustomerViewBasic)
                .put("Session/{id}/defer");
        assertEquals(HttpStatus.OK.value(), deferResponse.getStatusCode());
    }

    private void deferSessionExistingCustomerNoId(String token) {
        setUpDeferment();
        customer1.setInternationalCode("44");
        String phoneNumber = "7654567889";
        customer1.setPhoneNumber(phoneNumber);
        customerRepository.save(customer1);
        HostCustomerViewBasic hostCustomerViewBasic = new HostCustomerViewBasic();
        hostCustomerViewBasic.setPhoneNumber("0"+phoneNumber);
        doDefer(token, hostCustomerViewBasic);
    }

    private void setUpDeferment() {
        session1.setRestaurantId(restaurant1.getId());
        ...
        order1.setDinerId(diner.getId());
        order2.setDinerId(diner.getId());
        order3.setDinerId(diner.getId());
        orderRepository.save(order1);
        orderRepository.save(order2);
        orderRepository.save(order3);
    }

    private Response copyDeferred(String token) {
        Response copyResponse = given()
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .header(Params.AUTHORIZATION, token)
                .pathParam("id", session1.getId())
                .put("Session/{id}/copyDeferred");
        assertEquals(HttpStatus.OK.value(), copyResponse.getStatusCode());
        return copyResponse;
    }

    @Test
    public void getDeferredSessions1() throws Exception {
        String token = getTokenForStaff(staff1);
        ...
        Response response = given()
                ...
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .header(Params.AUTHORIZATION, token)
                .get("Session/deferred");
        assertEquals(HttpStatus.OK.value(), response.getStatusCode());
        HostDeferredSession[] sessions = response.getBody().as(HostDeferredSession[].class);

        assertEquals(0, sessions.length);
    }

    @Test
    public void getDeferredSessions2() throws Exception {
        String token = getTokenForStaff(staff1);
        deferSession(token);

        assertGetDeferred(token);
    }

    @Test
    public void getDeferredSessions3() throws Exception {
        String token = getTokenForStaff(staff1);
        deferSession(token);
        // post a cashup request
        ...
        assertNotNull(sessionService.findSession(session1.getId()).getCashUpId());

        assertGetDeferred(token);
    }

    private void assertGetDeferred(String token) {
        Response response = given()
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .header(Params.AUTHORIZATION, token)
        ...
        HostDeferredSession deferred = sessions[0];
        assertEquals(session1.getId(), deferred.getSessionId());
        assertFalse(deferred.isPaid());
        assertEquals(staff1.getId(), deferred.getStaffId());
        assertEquals(.6, deferred.getRemainingTotal(), 0.01);
        assertEquals(customer1.getId(), deferred.getCustomer().getId());
    }
}
