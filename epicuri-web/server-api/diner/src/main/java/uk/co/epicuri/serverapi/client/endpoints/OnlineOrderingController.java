...
public class OnlineOrderingController {
    ...
    public ResponseEntity<?> acquireToken(@RequestParam("restaurantId") String restaurantId, @RequestParam("publicToken") String publicToken, HttpServletRequest request) {
        ...
        if(restaurant.getIntegrations().containsKey(ExternalIntegration.CAYMAN_GATEWAY)) {
            response.setCaymanAPIKey(restaurant.getIntegrations().get(ExternalIntegration.CAYMAN_GATEWAY).getKey());
            response.setCaymanAuth(restaurant.getIntegrations().get(ExternalIntegration.CAYMAN_GATEWAY).getToken());
        }
    ...
    public ResponseEntity<?> getTimeSlots(@RequestHeader(Params.AUTHORIZATION) String token,
        ...
        for(String time : bookingService.getAllTakeawayTimeSlots()) {
            long utcTime = TimeUtil.toEpochMillisDayOffsetFromToday(time, restaurant.getIANATimezone(), day);
    ...
    public ResponseEntity<?> createTakeaway(@NotNull @RequestBody CustomerTakeawayOrderRequest order) {
        order.setInstantiatedFromId(ActivityInstantiationConstant.ONLINE.getId());
        if(order.getItems() != null) {
            order.getItems().forEach(o -> o.setInstantiatedFromId(ActivityInstantiationConstant.ONLINE.getId()));
        }
        try {
