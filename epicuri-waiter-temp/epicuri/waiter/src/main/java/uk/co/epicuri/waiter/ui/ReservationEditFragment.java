...
//		}
	...
	private void submit(){
		if(null == reservation){
			createReservation(partyName.getText(), phoneNumber.getText(), numberInParty.getValue(), reservationTime.getTime(), notes.getText(), newCustomer);
		} else {
	...
	private void createReservation(CharSequence partyName,
			...
			CharSequence notes, EpicuriCustomer customer) {
		WebServiceCall call = new CreateEditReservationWebServiceCall(partyName, phoneNumber, numberInParty, date, notes, customer, true, omitCheck.isChecked());
		WebServiceTask task = new WebServiceTask(getActivity(), call, true);
