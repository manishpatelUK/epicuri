...
public class FuseBoxAggregationProxy {
    ...
    private boolean isClosed() {
        ...
        if(openingHours.getHours().containsKey(dayOfWeek)) {
            ...
            boolean foundInsideOpeningHours = false;
            for(HourSpan hourSpan : hourSpans) {
                ...
                if(hourSpan.getHourClose() == 24) {
                    //just set to 23:00 (because can't use 24), add a day then set to beginning of day
                    close = ZonedDateTime.of(localDT.getYear(), localDT.getMonth().getValue(), localDT.getDayOfMonth(), 23, 0, 0, 0, restaurantTz);
                    close = close.plusDays(1).toLocalDate().atStartOfDay(restaurantTz);
                } else {
                ...
                if (open.toEpochSecond() == localDT.toEpochSecond() || (open.isBefore(localDT) && close.isAfter(localDT))) {
                    foundInsideOpeningHours = true;
                }
            }

            return !foundInsideOpeningHours;
        } else {
            return true;
        }
    }
