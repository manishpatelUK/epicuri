...
public class ReportingService {
    ...
    public List<AggregatedItemsReportLine> getAggregatedItemsReportLines(ReportingConstraints reportingConstraints) {
        ...
        Map<String, Session> sessionMap = cleanedSessionsAndOrders.getA().stream().collect(Collectors.toMap(Session::getId, Function.identity()));
        ...
        for(Map.Entry<String,OrderAggregate> entry : reportingConstraints.isAggregateByPLU() ? ordersByMenuItemPLU.entrySet() : ordersByMenuItem.entrySet()) {
            ...
            line.setItemName(wrapQuotes(reportingConstraints.isAggregateByPLU() ? "MIXED" : item.getName()));
            line.setPrice(reportingConstraints.isAggregateByPLU() ? "MIXED" : String.format("%.2f", MoneyService.toMoneyRoundNearest(item.getPrice())));
            ...
            line.setType(wrapQuotes(reportingConstraints.isAggregateByPLU() ? "MIXED" : item.getType().toString()));
            line.setTaxName(reportingConstraints.isAggregateByPLU() ? "MIXED" : taxRateMap.get(item.getTaxTypeId()).getName());
            line.setTaxRate(reportingConstraints.isAggregateByPLU() ? "MIXED" : String.format("%.2f",taxRateMap.get(item.getTaxTypeId()).getRate()/10D) + "%");
    ...
    public List<CashUpReportLine> getCashupLines(ReportingConstraints reportingConstraints) {
        ...
        for(CashUp cashUp : cashUps) {
            ...
            line.setTips(String.format("%.2f",MoneyService.toMoneyRoundNearest(cashUp.getReport().get(CashUpKeys.TOTAL_TIP))));
