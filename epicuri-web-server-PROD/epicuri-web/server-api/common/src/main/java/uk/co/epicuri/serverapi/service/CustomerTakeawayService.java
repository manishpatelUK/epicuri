...
public class CustomerTakeawayService {
    ...
    public CustomerReservationCheck checkTakeawayRequest(CustomerTakeawayOrderRequest order, String customerId) {
        ...
        if(StringUtils.isNotBlank(order.getTimeSlot()) && order.getRequestedTime() == 0) {
            order.setRequestedTime(TimeUtil.toEpochMillisToday(order.getTimeSlot(), restaurant.getIANATimezone()) / 1000);
        }
    ...
    public boolean hitsBlackout(CustomerTakeawayOrderRequest request, String customerId) {
        // some basic checks before fusebox
        FuseBoxAggregationProxy fuseBoxAggregationProxy
                = customerId == null ?  FuseBoxAggregationProxy.createTakeawayProxy(autowireCapableBeanFactory, request)
                    : FuseBoxAggregationProxy.createTakeawayProxy(autowireCapableBeanFactory, request, customerId);
